...
BEGIN
CREATE TABLE [{schema}].[SchemaVersion](
...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[{schema}].[DF_SchemaVersion_CreatedOn]') AND type = 'D')
BEGIN
ALTER TABLE [{schema}].[SchemaVersion] ADD  CONSTRAINT [DF_SchemaVersion_CreatedOn]  DEFAULT (getutcdate()) FOR [CreatedOn]
END
GO
SET ANSI_NULLS ON
GO
